...
本研究では、画像のクラス分類における、ドメイン適応に取り組む。特に、トレーニングデータのドメインと比較して、テストデータのドメインのラベル付き画像が少数のみ状態における問題を扱う。ドメイン適応の手法として、トレーニングセットのデータをテストセットのドメインにスタイル転送を行うことによって、テストデータのドメインに近い特徴を持つ画像を生成する。そして、その画像を用いて、画像のクラス分類を行う畳み込みニューラルネットワークの学習を行う。この手法によって、トレーニング時とテスト時のドメイン差が緩和され、テストデータのドメインに対する画像のクラス分類の精度が向上することを期待する。

 本研究では、スタイル転送を、Leon A. Gatysによってニューラルスタイル転送(Neural Style Transfer, NST)によって提案された手法を用いて行う。
