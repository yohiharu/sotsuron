...
まずコンテンツ画像とスタイル画像を、それぞれ、事前学習された畳み込みニューラルネットワークに入力し、特徴マップを抽出する。これらの特徴は、層ごとに捉えている情報が異なる点に特徴があり、浅い層ではエッジや色のような低次特徴、深い層では物体の構造や形状のような高次特徴が得られる。この性質を利用することによって、コンテンツとスタイルの役割を明確に分担させることが可能となる。
