...
本研究の目的は、画像のクラス分類における半教師ありドメイン適応の課題に対し、ニューラルスタイル転送を用いたデータ生成が分類精度に与える影響を明らかにすることである。特に、スタイル画像を複数用いるための損失関数の拡張、画像生成過程における損失関数の切り替え、ならびに初期画像の違いが生成画像と分類性能に及ぼす効果を検証する。さらに、ニューラルスタイル転送によるデータ生成と古典的なデータ拡張手法を組み合わせた場合の有効性についても評価し、限られたターゲットドメインのラベル付きデータしか利用できない状況において、分類性能を向上させるための知見を得ることを目指す。
<!--
