...
ニューラルスタイル転送(Neural Style Transfer, NST)によって、トレーニング時のデータセットを、テスト時のデータセットのドメインのように変換し、新たな画像を生成する。この手法によって、トレーニング時とテスト時のドメイン差が緩和され、テストデータのドメインに対する分類性能が向上することを期待する。」そして、その画像を用いてモデルの学習を行う。そして、画像認識の精度が向上を目指す。
