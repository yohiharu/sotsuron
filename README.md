...
まずコンテンツ画像とスタイル画像を、それぞれ、事前学習された畳み込みニューラルネットワークに入力し、特徴マップを抽出する。得られた特徴マップは$`F^{l}_{ij}`$と表す。lは層の位置を、iとjはフィルタの位置を表す。畳み込み層では、層ごとに捉えている情報が異なり、浅い層ではエッジや色のような低次特徴、深い層では物体の構造や形状のような高次特徴が得られる。この性質を利用することによって、コンテンツとスタイルの役割を分担させることが可能となる。
