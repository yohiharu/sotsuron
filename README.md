...
このような問題に対応する課題は、ドメイン適応として知られている。

本研究では、画像のクラス分類のタスクにおける、ドメイン適応の問題に取り組む。ニューラルスタイル転送(Neural Style Transfer, NST)によって、トレーニング時のデータセットを、テスト時のデータセットのドメインのように変換し、新たな画像を生成する。そして、その画像を用いてモデルの学習を行う。そして、画像認識の精度が向上を目指す。
