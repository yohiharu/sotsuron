...
ここで、本研究で扱う問題設定では、スタイル画像が1枚に限定されるとは限らず、複数枚存在する状況が想定される。また、ニューラルスタイル転送では、勾配計算の際に、同一の損失関数を必ずしも用いる必要はないと考えられる。そこで、複数のスタイル画像を用いて生成した画像を学習データとして利用する場合、また、最適化過程における損失関数の扱い方を変化させた場合、それぞれ分類精度がどのように変化するか評価する。
...
### データ拡張手法
Leon A. Gatysらが提案したニューラルスタイル転送では、一般に、スタイル画像を1枚のみ用いるそこで、本研究では、複数のスタイル画像を用いたスタイル転送を可能とするために、損失関数の拡張を行う方法を提案する。
