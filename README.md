...
### 研究背景
畳み込みニューラルネットワーク(convolutional neural network, CNN)によって、従来に比べて、飛躍的に画像認識タスクの精度が向上してきた。畳み込みニューラルネットワークが飛躍的な精度の向上を達成した理由として、畳み込み層と呼ばれる特徴を抽出する層が、重要な役割を果たしている点がある。

しかし、畳み込みニューラルネットワークを用いる際に発生する問題点として、トレーニング時とテスト時でのドメインの違いによる精度の低下がある。ドメインとは、データの特性が異なる環境や条件を指す。例えば、自然画像のデータセットで学習されたモデルでは、手書きのイラストに対しては、精度が低下してしまうことが、考えられる。

このような問題に対応する課題は、ドメイン適応として知られている。ドメイン適応の手法として、〇〇や〇〇が知られている。
...
ニューラルスタイル転送(Neural Style Transfer, NST)によって、トレーニング時のデータセットを、テスト時のデータセットのドメインのように変換し、新たな画像を生成する。この手法によって、〇〇であることを、期待する。そして、その画像を用いてモデルの学習を行う。そして、画像認識の精度が向上を目指す。
...
ドメイン適応のための手法として、ソースドメインとターゲットドメインの特徴分布の差異が性能低下の原因であることから、○○という方法が、提案されてきた。
...
以下の図は、勾配計算をする際に用いる損失関数を毎回同じにする方法と、損失関数を適切に切り替えながら勾配計算を行う方法、それぞれ表している。
...
本研究で画像分類を行う畳み込みニューラルネットワークは、VGG16を基盤とし、、ImageNetで事前学習された重みを用いた畳み込み層部分を特徴抽出器として利用し、そのパラメータを固定した上で、新たに全結合層および分類層を付加したものとする。図は実際のネットワークの構成を示している。
...
複数枚のスタイル画像を用いる方法は、提案手法の章で説明したように、勾配の計算の際にスタイル画像をすべて損失
ここでは、それらの方法をそれぞれ用い画像を生成し、それぞれの手法によるクラス分類の精度を測定し、
