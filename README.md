...
### 実験環境
実験環境として、GPUを搭載している計算機を用いる。この計算機では、ニューラルスタイル転送による画像生成と、画像分類のための畳み込みニューラルネットワークの学習および推論の両方を行う。図は、実験で実際に用いる計算機のGPU、CPU、メモリ、OSを表す。GPUは、並列の数値計算を高速に行うことができる。ディープラーニングでは並列の計算を多く行うため、GPUを用いることによって、高速化を達成することができる。こ
