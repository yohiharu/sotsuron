...
### 基本方針
本研究は、トレーニングセットとテストデータが異なるドメインに属しており、特にテストデータはラベル付き画像が少ない状況である、半教師あり学習の問題設定とする。そこで、複数のドメインで構成されているラベル付き画像データセットを用いる。

トレーニングセットの画像を、テストセットの画像を用いてスタイル転送することで、ターゲットドメインの反映した新たな学習用画像を生成する。スタイル転送の手法として、Leon A. Gatysらが提案したニューラルスタイル転送の手法を用いる。
...
本研究で行うすべての実験は、この一連の処理の流れを基本構成とする。各実験では、条件の一部の要素のみを変更することで、それぞれの手法が分類精度に与える影響を調べる。
...
今後の課題として、まず、提案手法の有効性を高めるために、より高速に画像を生成することができるスタイル転送技術を用いてデータ生成を行った場合の、クラス分類の精度の評価を行う点が挙げられる・
