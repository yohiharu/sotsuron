...
医療診断では、異なる医療機器間で生じる画像の質感の差異における問題が、考えられる。より広く用いられている機器で取得されたデータをコンテンツ画像として利用し、
また、リモートセンシングでは、天候や季節、時間帯によって変化する地表の見た目の差の問題が考えられる。データがあまり取得できない条件下で観測された画像に対しても、。
